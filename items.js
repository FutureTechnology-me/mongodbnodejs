...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
		...
		this.db.collection('item').aggregate([{$group:{"_id":"$category","num":{$sum:1}}},{$sort:{"_id":1}}]).toArray(
			
			function(err, categories) {
				assert.equal(err, null);
				//console.log(categories);
				for (var i = 0; i < categories.length; i++) { 
					if(category.num == 9999)
						category.num = 0;
					category.num += categories[i].num ;
				}
				//console.log("current tot:"+category.num);
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
		...
		this.db.collection('item').find(query).sort({"_id":1}).limit(itemsPerPage).skip(page*itemsPerPage).toArray(
    ...
    this.getNumItems = function(category, callback) {
		...
		var items=null;
		this.db.collection('item').aggregate([{$match:query},{$group:{"_id":null,"sum":{$sum:1}}}]).toArray(
			
			function(err, items) {
				numItems = items[0].sum;
				assert.equal(err, null);
				//console.log("Final numItems item")
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
		...
		//console.log(query);
		//console.log("------------");
		this.db.collection('item').find({ $text : { $search : query } }).sort({"_id":1}).limit(itemsPerPage).skip(page*itemsPerPage).toArray(
			function(err, items) {
				assert.equal(err, null);
				//console.log("Final search items")
				//console.log(items);
				callback(items);
			});
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
		...
		this.db.collection('item').find({ $text : { $search : query} }).toArray(
			function(err, items) {
				assert.equal(err, null);
				numItems = items.length;
				//console.log("item search items length")
				//console.log(numItems);
				callback(numItems);
			});
    ...
    this.getItem = function(itemId, callback) {
		...
		console.log("itemId");
		console.log(itemId);
		this.db.collection('item').find({ "_id" : itemId }).toArray(
			function(err, item) {
				assert.equal(err, null);
				//numItems = items.length;
				console.log("item search for itemId");
				console.log(item);
				callback(item[0]);
			});
    }
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
		...
		, function(err, doc) {
				assert.equal(err, null);
				console.log("comment add for itemId")
				callback(doc);
		});
    }
