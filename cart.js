...
function CartDAO(database) {
    ...
    this.getCart = function(userId, callback) {
        ...
        */
		...
		console.log(userId);
		var userCart1=null;
		this.db.collection('cart').find({ "userId" : userId }).toArray(
			function(err, userCart1) {
				assert.equal(err, null);
				console.log("userCart1 items");
				for (var i = 0; i < userCart1[0].items.length; i++) {
					console.log(userCart1[0].items[i]);
					userCart.items.push(userCart1[0].items[i]);
				}
				callback(userCart);
			});
